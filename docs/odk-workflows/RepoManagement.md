...
Your ODK repositories configuration is managed in `src/ontology/OpenLHS-Imports-odk.yaml`. Once you have made your changes, you can run the following to apply your changes to the repository:
...
1. Add an import statement to your `src/ontology/OpenLHS-Imports-edit.owl` file. We suggest to do this using a text editor, by simply copying an existing import declaration and renaming it to the new ontology import, for example as follows:
    ```
    ...
    Ontology(<http://purl.obolibrary.org/obo/OpenLHS-Imports.owl>
    Import(<http://purl.obolibrary.org/obo/OpenLHS-Imports/imports/ro_import.owl>)
    Import(<http://purl.obolibrary.org/obo/OpenLHS-Imports/imports/go_import.owl>)
    ...
    ```
2. Add your imports redirect to your catalog file `src/ontology/catalog-v001.xml`, for example:
    ```
    <uri name="http://purl.obolibrary.org/obo/OpenLHS-Imports/imports/go_import.owl" uri="imports/go_import.owl"/>
    ```
...
in the catalog file to redirect the URL `http://purl.obolibrary.org/obo/OpenLHS-Imports/imports/go_import.owl`
...
1. remove the import declaration from your `src/ontology/OpenLHS-Imports-edit.owl`.
2. remove the id from your `src/ontology/OpenLHS-Imports-odk.yaml`, eg. `- id: go` from the list of `products` in the `import_group`.
...
The following change to your repo config (`src/ontology/OpenLHS-Imports-odk.yaml`) will switch the go import from an SLME module to a simple ROBOT filter module:
...
If you wish to customise your import entirely, you can specify your own ROBOT command to do so. To do that, add the following to your repo config (`src/ontology/OpenLHS-Imports-odk.yaml`):
...
Now add a new goal in your custom Makefile (`src/ontology/OpenLHS-Imports.Makefile`, _not_ `src/ontology/Makefile`).
...
1. Open `src/ontology/OpenLHS-Imports-odk.yaml`
...
1. Providing a `components/mycomp.owl:` make target in `src/ontology/OpenLHS-Imports.Makefile`
...
1. Providing an additional attribute for the component in `src/ontology/OpenLHS-Imports-odk.yaml`, `source`,
